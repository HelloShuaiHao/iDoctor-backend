...
from spine_checks import detect_spine_bbox, contains_sacrum_or_coccyx
...
def main(input_folder, output_folder):
    ...
    weight_all="outputall/model_final.pth"
    weight_weizhui="outputweizhui/model_final.pth"
    ...
    if image is None:
        print("sagittal 图像读取失败")
        return

    spine_bbox = detect_spine_bbox(image, weight_all)

    if spine_bbox is None:
        print("未检测到完整脊柱")
        return

    if not contains_sacrum_or_coccyx(image, spine_bbox, weight_weizhui):
        print("不包含尾椎/骶骨")
        return
...
if __name__ == "__main__":
    ...
    main("Dec/1443838", "Dec/1443838V7")
